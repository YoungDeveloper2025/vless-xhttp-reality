...
prompt_default() {
  ...
  if { exec 9<>/dev/tty; } 2>/dev/null; then
    printf '%s [%s]: ' "$label" "$default_value" >&9
...
main() {
  ...
  TMP_DIR="$(mktemp -d)"
  load_defaults
  SNI="$(prompt_default "SNI" "$DEFAULT_SNI")"
  PORT="$(prompt_default "Port" "$DEFAULT_PORT")"
  FINGERPRINT="$(prompt_default "uTLS fingerprint" "$DEFAULT_FINGERPRINT")"
  FINGERPRINT="${FINGERPRINT,,}"
  CLIENT_PATH="$(prompt_default "XHTTP path" "$DEFAULT_PATH")"
  MODE="$DEFAULT_MODE"
